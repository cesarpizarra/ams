  ...
  const handleLogin = async () => {
    try {
      ...
      const response = await axios.post(
        "https://lnhs-api.vercel.app/api/auth/login",
        {
          username,
          password,
        }
        ...
        <div className=" w-full flex items-center justify-center gap-20 bg-gray-100 p-8">
          <div className="hidden md:flex justify-center items-center w-96 animate__animated animate__zoomIn">
            <img
              src={LoginImage}
              alt="image"
              className="w-full mt-36 floating "
            />
          </div>
          <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-lg animate__animated animate__zoomIn">
